...
_COOKIE_ENV_VAR = 'SNSCRAPE_TWITTER_COOKIES'


def _load_auth_cookies(session: requests.Session) -> bool:
	cookie_path = os.getenv(_COOKIE_ENV_VAR)
	if not cookie_path:
		return False
	path = Path(cookie_path).expanduser()
	if not path.exists():
		_logger.warning(f'Cookie file {_COOKIE_ENV_VAR}={path} not found')
		return False
	try:
		...
		with path.open() as fp:
			for raw_line in fp:
				...
				if len(parts) == 7:
					...
					secure_flag = secure.lower() in ('true', '1')
					jar.set(name, value, domain=domain, path=cookie_path, secure=secure_flag)
				elif '=' in line:
					name, value = line.split('=', 1)
					jar.set(name.strip(), value.strip(), domain='.twitter.com', path='/')
				else:
					continue
		session.cookies.update(jar)
		ct0 = session.cookies.get('ct0')
		if ct0:
			session.headers.setdefault('x-csrf-token', ct0)
		return bool(session.cookies.get('auth_token'))
	except Exception as e:
		_logger.warning(f'Failed to load cookies from {path}: {e}')
		return False
...
class _TwitterAPIScraper(snscrape.base.Scraper):
	def __init__(self, baseUrl, *, guestTokenManager = None, maxEmptyPages = 0, **kwargs):
		...
		self._using_auth_cookies = _load_auth_cookies(self._session)
		if self._using_auth_cookies:
			_logger.info(f'Loaded authenticated cookies from {os.getenv(_COOKIE_ENV_VAR)}')
			ct0 = self._session.cookies.get('ct0')
			if ct0:
				self._apiHeaders['x-csrf-token'] = ct0
	...
	def _ensure_guest_token(self, url = None):
		if getattr(self, '_using_auth_cookies', False):
			return
